...
class HybridRetriever:
    ...
    def __init__(self):
        ...
        self.collection = chroma_client.get_or_create_collection(name = "my_collection",metadata={"hnsw:space": "cosine"})
    ...
    def add_documents(self, documents: List[Dict]):
        ...
        doc_id = [doc['id'] for doc in documents]
    ...
    def _vector_search(self, query: str, top_k: int) -> List[Dict]:
        ...
        results = self.collection.query(query_emb,n_results=top_k)

        # 1. 取出原始向量距离列表
        distances = results['distances'][0]  # 原始：[18.73, 28.29, 30.12] 等，越小越相关

        if distances:  # 避免空列表
            min_dist = min(distances)
            max_dist = max(distances)
            # 2. 原地修改 distances：把距离转成 0~1 相似度（越大越相关）
            for i in range(len(distances)):
                # 核心公式：(最大距离 - 当前距离) / (最大距离 - 最小距离) → 0~1
                results['distances'][0][i] = (max_dist - distances[i]) / (max_dist - min_dist + 1e-6)

        indices = [ int(i)-1 for i in results['ids'][0] ]
        ...
        return [
            {
                **self.documents[i],
                "score": results['distances'][0][cot]
            }
            for i,cot in zip(indices,count)
        ]
